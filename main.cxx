...
class BaseMesh
{
  ...
  public:
  BaseMesh(vector<size_t> const& vertices)
    : _vertices(vertices)
  {
    LOGFUNCTION;
  }
...
class MeshDelta
{
  ...
  public:
  bool has(size_t thing) const {
    LOGFUNCTION;
    return _entities.find(thing) != _entities.end();
  }
  bool isValid(size_t thing) const {
    LOGFUNCTION;
    assert(has(thing));
    return _entities.find(thing)->second;
  }
  void add(size_t thing, bool value=true) {
    LOGFUNCTION;
    assert(!has(thing));
...
class MeshView
{
  ...
  public:
  MeshView(BaseMesh const& baseMesh)
    : _baseMesh(baseMesh)
  {
    LOGFUNCTION;
  }
  void addDelta(shared_ptr<const MeshDelta> delta) {
    LOGFUNCTION;
...
class VertexAdder
{
  public:
    ...
    shared_ptr<const MeshDelta> execute(size_t thing) const {
      LOGFUNCTION;
...
void MeshView::iterate() const {
  // iterate over base mesh
  {
    auto findInDeltas = [&](decltype(*_baseMesh.begin()) thing) {
      return find_if(_deltas.begin(), _deltas.end(),
          [&](decltype(*_deltas.begin()) delta) {
            return delta->has(thing);
          }) != _deltas.end();
    };
    for_each(_baseMesh.begin(), _baseMesh.end(),
